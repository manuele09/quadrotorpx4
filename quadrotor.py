...
import numpy as np
import gurobipy
import relu_to_optimization as relu_to_optimization
import relu_system as relu_system
import geometry_transform as geometry_transform
import mip_utils as mip_utils
...
class QuadrotorWithPixhawkReLUSystem:
    ...
    def _add_dynamics_constraint_given_relu_bounds(
        ...
        # (dx[n+1] - dx[n], dy[n+1] - dy[n], dz[n+1] - dz[n], roll[n+1] - roll[n], # pitch[n+1] - pitch[n], yaw[n+1] - yaw[n])
        ...
        posdot_curr_coeff = torch.zeros((6, 3), dtype=self.dtype)
        posdot_curr_coeff[3:6, :] = -torch.eye(3, dtype=self.dtype)
        mip.addMConstr(
            [
                torch.eye(6, dtype=self.dtype), posdot_curr_coeff,
