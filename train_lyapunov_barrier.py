...
import inspect
import time
import hybrid_linear_system as hybrid_linear_system
import lyapunov as lyapunov
import barrier as barrier
import feedback_system as feedback_system
import utils as utils
import r_options as r_options
...
class Trainer:
    ...
    def __init__(self, tensorboard_log=False, log_dir="./Logs"):
        ...
        self.lyapunov_positivity_mip_cost_weight = 1.
        ...
        self.lyapunov_derivative_mip_cost_weight = 2.
        # The number of (sub)optimal solutions for the MIP
        # max_x dV(x) + εV(x)
    ...
    def solve_lyap_derivative_mip(self):
        dtype = self.lyapunov_hybrid_system.system.dtype
    ...
    def solve_barrier_value_mip(self, safe_flag):
        dtype = self.barrier_system.system.dtype
        regions = self.safe_regions if safe_flag else self.unsafe_regions
        mip = [None] * len(regions)
        mip_obj = [None] * len(regions)
        mip_adversarial = [None] * len(regions)

        for region_count, region in enumerate(regions):
            mip[region_count], x = self.barrier_system.value_as_milp(
                self.barrier_x_star, self.barrier_c, region, safe_flag)
            mip[region_count].gurobi_model.setParam(
                gurobipy.GRB.Param.OutputFlag, False)
            if self.barrier_value_mip_pool_solutions > 1:
                mip[region_count].gurobi_model.setParam(
                    gurobipy.GRB.Param.PoolSearchMode, 2)
                mip[region_count].gurobi_model.setParam(
                    gurobipy.GRB.Param.PoolSolutions,
                    self.barrier_value_mip_pool_solutions)
            mip[region_count].gurobi_model.optimize()
            mip_obj[region_count] = mip[region_count].gurobi_model.ObjVal
            num_adversarial = np.min((self.barrier_value_mip_pool_solutions,
                                      mip[region_count].gurobi_model.solCount))
            mip_adversarial[region_count] = torch.empty(
                (num_adversarial, self.barrier_system.system.x_dim),
                dtype=dtype)
    ...
    def train(self, state_samples_all):
        ...
        while iter_count < self.max_iterations:
            ...
            if self.output_flag:
                ...
                file1 = open("Test_26_12_2023_pixhawk_quad/Derivative.txt","a")
                file1.write(str(total_loss_return.lyap_loss.derivative_mip_obj))
                file1.write("\n")
                file1.close()

                file2 = open("Test_26_12_2023_pixhawk_quad/Positivity.txt","a")
                file2.write(str(total_loss_return.lyap_loss.positivity_mip_obj))
                file2.write("\n")
                file2.close()

                file3 = open("Test_26_12_2023_pixhawk_quad/Time.txt","a")
                file3.write(str(time.time() - train_start_time))
                file3.write("\n")
                file3.close()

                file4 = open("Test_26_12_2023_pixhawk_quad/Loss.txt","a")
    ...
    def train_lyapunov_on_samples(self, state_samples_all, num_epochs,
        ...
        for epoch in range(num_epochs):
            ...
            positivity_loss = 0.
            derivative_loss = 0.
            test_positivity_loss = 0.
            test_derivative_loss = 0.
            ...
            test_positivity_loss = test_loss_return.lyap_loss.positivity_sample_loss.item() + test_loss_return.lyap_loss.positivity_mip_loss.item()
            test_derivative_loss = test_loss_return.lyap_loss.derivative_sample_loss.item() + test_loss_return.lyap_loss.derivative_mip_loss.item()
            
            if self.writer is not None:
                self.writer.add_scalar('samples_loss/train/total', running_loss, global_step=epoch)
                self.writer.add_scalar('samples_loss/train/positive', positivity_loss, global_step=epoch)
                self.writer.add_scalar('samples_loss/train/derivative', derivative_loss, global_step=epoch)
    
                self.writer.add_scalar('samples_loss/test/total', test_loss_return.loss.item(), global_step=epoch)
            ...
            if self.output_flag:
                print(f"TrainLoss -- Iter {epoch}, loss {running_loss}, " +
                      ...
                      "derivative_cost " +
                      f"{derivative_loss}")
                print(f"TestLoss -- Iter {epoch}, loss {test_loss_return.loss.item()}, " +
                      "positivity cost " +
